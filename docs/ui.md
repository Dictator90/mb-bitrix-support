...
> **⚠️ Примечание:** Построение форм, полей, таблиц и страниц настроек перенесено в отдельный пакет [`mb4it/bitrix-admin-kit`](https://github.com/mb4it/bitrix-admin-kit). В этом пакете остаются только **`UI\EntitySelector`** (провайдеры для диалогов выбора) и **`UI\Admin\MenuIcon`** (enum иконок меню).

Пакет `MB\Bitrix\UI` содержит провайдеры селекторов для диалогов выбора Bitrix и утилиты для интеграции с админ-панелью.
