...
### Кросс-СУБД (MySQL / PostgreSQL)

`MB\Bitrix\Storage\SqlHelper` и трейты массовых операций ориентированы на **MySQL** и **PostgreSQL**:

- **MySQL:** пакетное обновление использует конструктор `ROW(...)` внутри `UPDATE … JOIN (VALUES …)` — убедитесь, что версия MySQL/MariaDB в вашей среде это поддерживает.
- **PostgreSQL:** UPSERT строится через `ON CONFLICT`; при включённом обновлении дубликатов **должен** быть непустой список колонок первичного ключа в метаданных сущности — иначе выбрасывается `InvalidArgumentException` (см. unit-тесты `tests/Storage/SqlHelperTest.php`).
- **Пакетный upsert (`addBatch`):** набор колонок фиксируется по **первой** строке данных; строки с другим набором ключей могут дать некорректный SQL — нормализуйте ключи строк до вызова (см. `BatchUpsertTrait`).

> **См. также:** [UI-подсистема](ui.md) — для интеграции Highload-блоков с административными формами и гридами через EntityView.
